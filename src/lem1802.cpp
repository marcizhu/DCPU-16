...
LEM1802::LEM1802(DCPU16* c, uint16 delay) : Hardware(c, 0x7349f615, 0x1802, 0x1c6c8b36), delay(delay)
{
	...
	window = SDL_CreateWindow("LEM1802", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 128 * SCALE, 96 * SCALE, SDL_WINDOW_SHOWN);
...
void LEM1802::interrupt()
{
	switch(cpu->reg[A])
	{
		...
		case 4: for(int n = 0; n < 256; ++n) { cpu->mem[uint16(cpu->reg[B] + n)] = getFontCell(n, true); cpu->tick(1); } break;
		case 5: for(int n = 0; n <  16; ++n) { cpu->mem[uint16(cpu->reg[B] + n)] = getPalette(n,  true); cpu->tick(1); } break;
	}
}
...
{
	Uint8 r, g, b;
	uint16 backColor = getPalette(borderColor);
	...
	for(unsigned int y = 0; y < 12; y++)
	{
		...
		{
			uint16 v = cpu->mem[(uint16)(ramBase + x + y * 32)];
			...
			uint16 font[2] = { getFontCell(ch * 2 + 0), getFontCell(ch * 2 + 1) };
			...
			for(unsigned int yp = 0; yp < 8; yp++)
			{
				...
				{
					uint8 color = ((font[xp / 2] & (1 << (yp + 8 * ((xp & 1) ^ 1)))) ? fg : bg);
...
uint16 LEM1802::getPalette(unsigned int n, bool force) const
{
	static const uint16 palette[16] = { 0x000, 0x00A, 0x0A0, 0x0AA, 0xA00, 0xA0A, 0xAA5, 0xAAA, 0x555, 0x55F, 0x5F5, 0x5FF, 0xF55, 0xF5F, 0xFF5, 0xFFF };

	return (force || !paletteBase) ? palette[n] : cpu->mem[(uint16)(paletteBase + n)];
}

uint16 LEM1802::getFontCell(unsigned int n, bool force) const
{
	static const uint16 font4x8[256] =
		{ 
	...
	return (force || !fontBase) ? font4x8[n] : cpu->mem[(uint16)(fontBase + n)];
}